...
def process_csv(file_path, added_words_file='added_words.txt'):
    ...
    for _, row in new_words_df.iterrows():
        ...
        phonetic = str(row['phonetic']) if not pd.isna(row['phonetic']) else "NONE"
        translation = str(row['translation']) if not pd.isna(row['translation']) else "NONE"
        sentences = str(row['sentences']) if not pd.isna(row['sentences']) else "NONE"
